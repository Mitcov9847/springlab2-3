...
2. Настройте базу данных MSSQL и укажите параметры в `application.properties`:
   ```properties
...
После запуска приложения и перехода по адресу Swagger UI, вы увидите визуализированный интерфейс для работы с API. В Swagger доступны все методы, описанные в контроллерах, а также возможность отправлять HTTP-запросы.

Действия в Swagger:
Перейдите в раздел /authors, чтобы увидеть доступные методы для работы с авторами. Выберите метод GET /authors, чтобы получить список авторов.

Попробуйте создать нового автора. Выберите метод POST /authors и заполните форму:
...
Нажмите Execute, чтобы отправить запрос.

Проверьте созданного автора с помощью метода GET /authors/{id}, где введите ID нового автора, чтобы получить информацию о нем.

Скриншот интерфейса Swagger
Вставьте сюда скриншот с интерфейсом Swagger, показывающий доступные методы и результаты запросов.
